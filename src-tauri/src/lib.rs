...
#[tauri::command]
async fn update_tray_menu(app: tauri::AppHandle, show_main_text: String, quit_text: String) -> Result<(), String> {
    if let Some(tray) = app.tray_by_id("tauri") {
        let show_i = MenuItem::with_id(&app, "show_main", show_main_text, true, None::<&str>)
            .map_err(|e| e.to_string())?;
